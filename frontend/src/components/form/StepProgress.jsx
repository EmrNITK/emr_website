...
const StepProgress = ({ steps, currentStep, onStepClick, stepData = {} }) => {
  ...
  const isStepComplete = (index) => {
    // Check if the step has all required fields filled
    const data = stepData;
    switch (index) {
      case 0:
      ...
      case 2:
        return data.ruleBook?.trim() && data.amount > 0;
      case 3:
  ...
  const getStepSummary = (index) => {
    ...
    switch (index) {
      ...
      case 2:
        return data.ruleBook || data.amount ? "Rules & fees set" : "Rules & fees needed";
      case 3:
               ...
               <div key={index} className="flex items-center flex-1">
                 <div className="relative flex flex-col items-center">
                   ...
                   {hoveredStep === index && isClickable && (
                     <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-64 bg-black/90 backdrop-blur-sm rounded-lg shadow-xl border border-white/20 p-3 z-10">
            ...
            {steps.map((step, index) => {
             ...
             return (
               <button
                 ...
                 onClick={() => isClickable && onStepClick(index)}
                 disabled={!isClickable}
                                   className={`flex-1 mx-1 px-3 py-2 rounded-lg text-xs font-medium transition-all duration-200 ${
                    isStepComplete(index)
